...
model.fixed <- function(X,T1,Y,lambda1){
  ...
  norma_TETA <- 10 ; epsilon_TETA <- 10^-5
  norma_ALFA <- 10 ; epsilon_ALFA <- 10^-5
  norma_F1 <- 10 ; epsilon_F1 <- 10^-5
  
  while (norma_TETA >= epsilon_TETA){
    while(norma_ALFA >= epsilon_ALFA  & norma_F1 >= epsilon_F1){
      ...
      norma_ALFA <- sqrt(t(ALFA_en - ALFA_in)%*%(ALFA_en - ALFA_in)/(t(ALFA_in)%*%ALFA_in))
      norma_F1 <- sqrt(t(F1_en-F1_in)%*%(F1_en-F1_in)/(t(F1_in)%*%F1_in))
  ...
  return(list(AIC=AIC_alpha1,edf_func1=edf_alpha1,beta=ALFA_en
              ,F1_en=F1_en,phi=fi_en,SD_beta=SD_ALFA_n,SD_F1_n=SD_F1_n,SD_phi=SD_fi_n))
